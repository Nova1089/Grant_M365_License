...
function Grant-LicenseToUsers($userCsv, $license)
{
    Write-Host "Granting license: $($license.name)" -ForegroundColor $script:infoColor
    foreach ($user in $userCsv)
    {
        Write-Progress -Activity "Granting license to users..." -Status $user.UserPrincipalName
...
function Grant-License($user, $license)
{
    ...
    try
    {
        Set-MgUserLicense -UserId $upn -AddLicenses @{SkuId = $license.SkuId } -RemoveLicenses @() -ErrorAction "Stop" | Out-Null
    }
    catch
    {
        $errorRecord = $_
        Log-Warning "There was an issue granting M365 license: $($license.name) to $upn`n$errorRecord"
    }
}
...
Grant-LicenseToUsers -UserCsv $userCsv -License $licenseToGrant
